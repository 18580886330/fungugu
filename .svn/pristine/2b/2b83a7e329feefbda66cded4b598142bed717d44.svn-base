			...
			server.getHouseDetail(params).success(function(data){
					if(data.code==200){
						...
						yf.filterStr(data.data.details.greeningRate)=="--"?data.data.details.greeningRate="--":
																			data.data.details.greeningRate=parseInt(Number(data.data.details.greeningRate)*100);				
						$('#uptown_info').html('<tr>\
							                    	<td colspan="2"><strong>小区地址：</strong><span title="'+yf.userInfo.city+yf.$params().istrative+yf.filterStr(data.data.details.address)+'">'+yf.userInfo.city+'&nbsp'+yf.$params().istrative+'&nbsp'+yf.filterStr(data.data.details.address)+'</span></td>\
								                </tr>\
								                <tr>\

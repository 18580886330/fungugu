require([
    ...
    'src/intelligentSearch/houseValuation/js/searchPrompt.js'
], function(modules,server,searchPop) {
    'use strict';
    /**
     * 房产估值模块入口
     */
    var main = ({
        init: function() {
            ...
            this.competingFlag = false; //是否有参考均价 
            this.artificialFlag = false;//是否有人工核价的权限
            this.diffFlag = false;   //差异化描述滚动条
            yf.placeholder();
            return this;
        },
        data: function() {
            ...
            this.jPriceType = valuationResponse.find('#priceType').eq(0);  // 显示价格类型
            ...
            this.describe = $('#diff_describe');//差异化描述
            this.mortgagePrice;//抵押单价
            this.mortgageTotalPrice;//抵押总价
            this.vPrice;//市场单价
            this.vTotalPrice;//市场总价
        },
        // 左侧导航状态
        navEvent: function() {
        ...
        search: function() {
            ...
            this.valuationInput.autoprompting({
                ...
                /**
                 * data:返回数据
                 * sval:当默认选中第一条的时候返回用户输入的其他时候返回空字段
                 * len:搜索返回的长度（用来判断是不是房本地址）
                 * curWorld:任何时候都返回用户输入的字段(originalInputItem)
                 * */
                enter: function(data,sval,len,curWorld) {
                	//这里需要先判断输入项是否和返回的字段相匹配 如果不匹配则需要弹窗提示
                	if(sval&&sval!=data.keywords&&sval!=data.residentialname){
                		searchPop.init(data,sval,len,curWorld);
                		return;
                	}
                    if(len == 1){
                        ...
                        location.href = '#/houseValuation?alias='+(!data.similarword?data.residentialname:data.similarword)+'&housingName='+data.residentialname+'&istrative='+data.districtname+'&sval='+sval+'&originalInputItem='+curWorld;
                    }
                    location.reload();
                }
            });
        ...
        houseReferenceArea: function() {
            ...
            //默认加载第一个模块
            if($('#houseReference li').length<=0) return;
             //modules['referenceCase'](); // 参考案例
            var model=$('#houseReference ul').find('.layui-this').attr('data-module');
            modules[model]();
            ...
            yf.matchModule(moduleNames,function(data) {
               for(var i=0; i<data.length; i++) {
                    html += '<a href="javascript:;" class="layui-btn'+sClass+'" data-module="'+data[i].nameEn+'">'+data[i].nameZn+'</a>';
               		if(data[i].nameZn=='参考均价'){
               			_this.competingFlag = true;
               		};
               		if(data[i].nameZn=='人工核价'){
               			_this.artificialFlag = true;
               		};
               }
                _this.oValuationTool.html( html );
              	_this.multiBtnFn(data);
               	$(window).resize(function(){
               		_this.multiBtnFn(data);
               	});
                // 绑定事件，加载模块
                _this.oValuationTool.find('a[data-module]').not('.layui-disabled').each(function(idx,elm){
                    $(elm).unbind().bind('click',function() {
                    	// 如果点击参考均价 直接展示该模块
                    	if($(this).text()=="参考均价"){
                    		if($(this).hasClass('layui-disabled')) return;
                    		$('#valuation_result').hide();
                    		$('#competingPrice_result').show();
                    		return;
                    	}
        ...
        //右边模块换行
        multiBtnFn:function(data){
           	var vCont = this.oValuationTool.width();
           	var num=0;
           	for(var i =0;i<this.oValuationTool.find('a').length;i++){
           		num += Number(this.oValuationTool.find('a').eq(i).outerWidth());
           	}
           var tW = num+Number((data.length-1)*10);
           	if(vCont<=tW){
           		$('#valuationResponse').addClass('multi-result');
           	}else{
           		$('#valuationResponse').removeClass('multi-result');
           	}
        },
        // 最近搜索
        getRecords: function() {
            ...
            server.getRecords().success(function(data) {
                ...
                var data = data.data;
                for( var i=0; i<data.length; i++ ) {
                    sHtml += '<a href="javascript:;" data-similarword="'+data[i].comName+'" data-districtname="'+data[i].district+'" data-keywords="'+data[i].comName+'" title="'+data[i].comName+'" class="item">'+data[i].comName+'</a>';
                }
                oSearchLogs.append(sHtml).find('a').bind('click',function() {
                    location.href = '#/houseValuation?alias='+$(this).data().similarword+'&housingName='+$(this).data().keywords+'&istrative='+$(this).data().districtname+'&originalInputItem='+$(this).text();
                });
            }).error(function(e){});
        },
        getObjects: function() {
            ...
            oGugu.unbind('click').bind('click',function() {
                ...
                if( !/^[1-9]\d{0,3}(\.\d{0,2})?$/.test( sHouseArea ) || sHouseArea > 9998.99 ) {
                    ...
                    reqParams = {
                        ...
                        pointX: '',                               // 经度
                        pointY: '',                               // 纬度
                        originalInputItem:yf.param('originalInputItem'),//用户原始输入项  
                    }
                    server.gugu(reqParams).success(function(data){
                        ...
                        //人工核价
                        _this.showArtificialPop();
                         //参考均价
            			_this.competingData();
                        oGugu.addClass('disabled');         // 重复点击拦截
                        yf.removeLoading('#valuationResponse'); // loading
						yf.taxationNum='';//重置税费值                        
                        setTimeout(function(){
        ...
        responseData: function(data) {
            ...
            //显示价格类型
            switch(data.priceTypeShow){
            	//市场评估价
            	case '0':
            		_this.jPriceType.text(yf.priceTypeShow[data.priceTypeShow]);
            		_this.jUnitPrice.text(parseInt(data.vPrice)).next().show();
            		_this.jTotalPrice.text(parseInt(data.vTotalPrice)).next().show();
            		break;
            	case '1':
            		_this.jPriceType.text(yf.priceTypeShow[data.priceTypeShow]);
            		_this.jUnitPrice.text(parseInt(data.mPrice)).next().show();
            		_this.jTotalPrice.text(parseInt(data.mTotalPrice)).next().show();
            		break;
            	case '2':
            		_this.jPriceType.text(yf.priceTypeShow[data.priceTypeShow]);
            		_this.jUnitPrice.text(parseInt(data.vPrice*data.mortgageRate)).next().show();
            		_this.jTotalPrice.text(parseInt(data.vTotalPrice*data.mortgageRate)).next().show();
            		break;	
            }
            this.mortgagePrice = data.mPrice;
            this.mortgageTotalPrice = data.mTotalPrice;
            this.vPrice = data.vPrice;
            this.vTotalPrice = data.vTotalPrice;
            this.jTimer.text(data.valuationTime);
            this.jDiscount.prop('disabled',false).val(parseInt(data.discountRate*100));
            this.jSumPrice.text( parseInt(data.vTotalPrice*data.discountRate) );
			//差异描述
			if(data.differenceExplanation){
				this.describe.removeClass('hide');
				$('#describe_covers').text(data.differenceExplanation);
				yf.menuDown({node:$('#diff_tub'),showNode:$('#diff_cont')});
                if(!this.diffFlag){
                    // yf.scroll({elmId: '#describe_covers'});
                    $('#describe_covers').mCustomScrollbar({
        ...
        //参考均价
        competingData: function(){
        	//是否有竞品权限
        	if(!this.competingFlag) return;
        	var _this = this;
        	var params = {
        		  comName: yf.param('housingName')// 小区名称(必填)
        	};
        	this.backToValuation();
        	server.getCompetingPrice(params).success(function(data){
        		if(data.code==200){
        			var str = '';
        			var arr=[];
        			var avg;//平均值
        			var curPrice = _this.jUnitPrice.text() =="暂无价格" ? 0:_this.jUnitPrice.text();
        			for(var i=0;i<data.data.length;i++){
        				arr.push(Number(data.data[i].price));
        			};
        			//var num = Number(data.data.caseDate.split('-')[1]);
        		
        			//$('#competing_month').text(data.data.caseDate);
        			//添加当前估值的价格
        			arr.push(Number(curPrice));
        			avg=Math.max.apply(null, arr);
        			for(var i=0;i<data.data.length;i++){
        				str+='<tr>\
        						<td>'+data.data[i].source+'</td>\
        						<td><div class="layui-progress layui-progress-big" lay-showpercent="true">\
                           			 	<div class="layui-progress-bar layui-bg-blue" lay-percent="100%"  style="width:'+Math.floor((Number(data.data[i].price)/avg)*100)+'%" ></div>\
                   					</div>\
                       			</td>\
        			...
        			if(curPrice>0){
        				str+='<tr>\
        						<td><p>估值结果</p></td>\
        						<td><div class="layui-progress layui-progress-big" lay-showpercent="true">\
                           			 	<div class="layui-progress-bar layui-bg-red" lay-percent="100%"  style="width:'+Math.floor((Number(curPrice)/avg)*100)+'%" ></div>\
                   					</div>\
                       			</td>\
                       			<td>'+curPrice+'元/㎡</td>\
        					</tr>'
        			...
        			$('#competing_list').html(str);		
        			layui.use(['element'], function(){
  				  		var element = layui.element;
				 	});	
				 	_this.oValuationTool.find('a').each(function(idx,elm){
    					if($(elm).text()=="参考均价"){
    						$(elm).removeClass('layui-disabled');
    						//添加点击事件
    						$(elm).click(function(){
	    							if($(this).text()=="参考均价"){
		                    		$('#valuation_result').hide();
        			...
        			_this.oValuationTool.find('a').each(function(idx,elm){
    					if($(elm).text()=="参考均价"){
    						var popHtml = $("<div class='pop-hint-text' id='non_competition'>该小区暂无参考均价<div class='pa arrorw-down'><i class='pa upward'></i></div></div>");
    						popHtml.appendTo($('#valuationResponse'));
    						yf.menuDown({node:$(elm),showNode:$('#non_competition')});
    					}
        			});
        			$('#competingPrice_result').hide();
        			$('#valuation_result').show();
        		}
        	}).error(function(){})
        },
        //展示人工核价
        showArtificialPop:function(){
        	if(!this.artificialFlag) return;
			this.oValuationTool.find('a').each(function(idx,elm){
				if($(elm).text()=="人工核价"){
					$(elm).removeClass('layui-disabled');
					//添加点击事件
					$(elm).off('click').click(function(){
						searchPop.goToArtificial();
					})
				}
			})			        	
        },
        //参考均价上的返回
        backToValuation:function(){
        	$('#back_result').off('click').on('click',function(){
        		$('#competingPrice_result').hide();
        		$('#valuation_result').show();
        	})
        }
    ...
    return yf.$params = (function(){
        return {
            ...
            jUnitPrice: main.vPrice || '',         // 市场单价
            jTotalPrice: main.vTotalPrice || '',       // 市场总价
            ...
            mortgagePrice:main.mortgagePrice||'',//抵押单价
            mortgageTotalPrice:main.mortgageTotalPrice||'' //抵押总价
        };
    });

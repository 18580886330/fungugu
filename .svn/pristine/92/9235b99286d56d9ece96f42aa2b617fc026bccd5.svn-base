...
(function( $ ) {
    $.fn.extend({
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
			...
			if(!webUploader) return;//浏览器要是不支持 直接返回
			
			//加入队列之前
			webUploader.on('beforeFileQueued',function(file){
				/*--------------分界线----------------------*/
				var obj=$('#'+$fileInputId).siblings('.parentFileBox').find('ul li');
				if(obj.length>=2){
					layer.msg('图片上传超过限制');
					return false;
				}
			...
			webUploader.on('fileQueued', function( file ) {
				createBox( $fileInput, file ,webUploader,$fileInputId);
			});
			...
			webUploader.on('uploadSuccess',function( file, response ){
				...
				$fileBox.removeAttr('data-reup');
				$($fileBox).attr('data-fileName',response.data.fileName);
				$($fileBox).attr('data-valid',response.data.isThisUpload)
				$diyBar.fadeOut( 1000 ,function(){
					$fileBox.children('.diyCancel').hide();//隐藏取消按钮
			...
			webUploader.on('uploadError',function( file, response ){
				...
				var err = '上传失败! 文件:'+file.name+' 错误码:'+response;
				if ( errorCallBack ) {
					errorCallBack( err );
				}
			});
	...
	function getOption(objId) {
		...
		*/
		...
		return {
			swf:'/src/content/js/third/webUpload/Uploader.swf',
			//按钮容器;
			pick:{
				id:objId,
				label:"点击选择图片"
			...
			accept:{
				 title: 'Images',
  				 ...
  				 mimeTypes: 'image/bmp,image/jpg,image/jpeg,image/png'
			},
			//配置生成缩略图的选项
			thumb:{
				width:110,
	...
	//创建文件操作div;	
	function createBox( $fileInput, file, webUploader,$fileInputId ) {
		...
		$('.diyButton').hide();
		//添加子容器;
		var li = '<li id="fileBox_'+file_id+'" class="diyUploadHover" data-reup="true"> \
		...
		/*只是替换图片*/
		updataPicLi('chooseHead',webUploader);
		updataPicLi('chooseJrcode',webUploader);
		updataPicLi('license',webUploader);
		updataPicLi('credit',webUploader);
		...
		if ( file.type.split("/")[0] != 'image' ) {
			...
			var addFileType = getFileTypeClassName( file.name.split(".")[1]);
			/*if(addFileType != "jpg" || addFileType != "jpeg" || addFileType != "png" || addFileType != "gif" || addFileType != "bmp"){
				alert("只能上传图片！");
	...
	//替换图片
	function updataPicLi($fileInputId,webUploader){
		var len=$('#'+$fileInputId).siblings('.parentFileBox').find('.fileBoxUl li').length;
		if(len>1){
			var id=$('#'+$fileInputId).siblings('.parentFileBox').find('.fileBoxUl li').eq(0).attr('id').split('fileBox_')[1];
			$('#'+$fileInputId).siblings('.parentFileBox').find('.fileBoxUl li').eq(0).remove();
			webUploader.removeFile(id);
		}
